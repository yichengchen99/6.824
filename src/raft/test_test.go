...
//
//	DPrintf("TEST: disconnect %v", leader1)
//	// if the leader disconnects, a new one should be elected.
//	cfg.disconnect(leader1)
//	cfg.checkOneLeader()
//
//	DPrintf("TEST: connect %v", leader1)
//	// if the old leader rejoins, that shouldn't
//	// disturb the new leader.
//	cfg.connect(leader1)
//	leader2 := cfg.checkOneLeader()
//
//	DPrintf("TEST: disconnect %v", leader2)
//	DPrintf("TEST: disconnect %v", (leader2+1)%servers)
...
//	cfg.checkNoLeader()
//
//	DPrintf("TEST: connect %v", (leader2+1)%servers)
//	// if a quorum arises, it should elect a leader.
//	cfg.connect((leader2 + 1) % servers)
//	cfg.checkOneLeader()
//
//	DPrintf("TEST: connect %v", leader2)
...
func TestRejoin2B(t *testing.T) {
	...
	cfg.begin("rejoin of partitioned leader")

	DPrintf("TEST_ACTION cmd 101")
	cfg.one(101, servers, true)

	// leader network failure
	leader1 := cfg.checkOneLeader()
	DPrintf("TEST_ACTION disconnect [%v]", leader1)
	cfg.disconnect(leader1)

	// make old leader try to agree on some entries
	DPrintf("TEST_ACTION start 102, 103, 104 at [%v]", leader1)
	cfg.rafts[leader1].Start(102)
	cfg.rafts[leader1].Start(103)
	cfg.rafts[leader1].Start(104)

	// new leader commits, also for Index=2
	DPrintf("TEST_ACTION cmd 103")
	cfg.one(103, 2, true)

	// new leader network failure
	leader2 := cfg.checkOneLeader()
	DPrintf("TEST_ACTION disconnect [%v]", leader2)
	cfg.disconnect(leader2)

	// old leader connected
	DPrintf("TEST_ACTION connect [%v]", leader1)
	cfg.connect(leader1)

	DPrintf("TEST_ACTION cmd 104")
	cfg.one(104, 2, true)

	// all together now
	DPrintf("TEST_ACTION connect %v", leader2)
	cfg.connect(leader2)

	DPrintf("TEST_ACTION cmd 105")
...
//
//	// submit lots of commands that won't apply
//	for i := 0; i < 50; i++ {
...
//
//	// lots more commands that won't apply
//	for i := 0; i < 50; i++ {
...
func internalChurn(t *testing.T, unreliable bool) {
	...
	cfn := func(me int, ch chan []int) {
		...
		for atomic.LoadInt32(&stop) == 0 {
			...
			if ok {
				// maybe leader will apply our value, maybe not.
